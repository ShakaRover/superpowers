...
# Run Codex with subagent-driven-development
...
echo "================================================================================"
set +e
cd "$SCRIPT_DIR/../.." && printf '%s' "$PROMPT" | timeout 1800 env CODEX_HOME="$SCRIPT_DIR/.codex-home" "$SCRIPT_DIR/../.."/commands/codex-auto.sh --max-retries 2 -- exec --full-auto -C "$SCRIPT_DIR/../.." --add-dir "$TEST_PROJECT" --output-last-message "$OUTPUT_FILE" --json > "$EVENTS_FILE" 2> "$LOG_FILE"
exit_code=$?
set -e
...
if [ ! -f "$EVENTS_FILE" ]; then
    echo "ERROR: Could not find Codex events file"
    echo "Expected: $EVENTS_FILE"
    exit 1
fi

echo "Analyzing events: $(basename "$EVENTS_FILE")"
echo ""
...
echo "========================================="
echo ""
python3 "$SCRIPT_DIR/analyze-token-usage.py" "$EVENTS_FILE"
echo ""
